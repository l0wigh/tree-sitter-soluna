...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_soluna", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_soluna": ["*.pyi", "py.typed"],
        "tree_sitter_soluna.queries": ["*.scm"],
    },
    ext_package="tree_sitter_soluna",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_soluna/binding.c",
                "src/parser.c",
            ],
            define_macros=[
